...
async function sendLocalCreateTx() {
	...
	formData.append("name", "AI Cat");
	formData.append("symbol", "$CAT");
	formData.append("description", "THE OFFICIALCat Simulator ON SOLAANA.");
	...
	if (response.status === 200) {
		...
		const lp_v2 = {
			...
			baseMint_Me: mintKeypair.publicKey.toBase58(),
					...
					for (const predictionDoc of PredictionData) {
						try {
							...
							if (side === 'Buy') {
								...
								const ammo = 0.01
								// const baseAmmo = 0.005; // Base amount to buy in SOL
								// let ammo = baseAmmo; 

								// if (liquidityPrediction > 5000) {
								// 	ammo *= 2; // Double the buy amount if liquidity is high
								// }

								// if (pricePrediction > price * 1.1) {
								// 	ammo *= .5; // Increase amount if predicted price is 10% higher than current price
								// }

								// if (dynamicSlippage > 0.05) {
								// 	ammo *= 0.25; // Reduce buy amount if slippage is high
								// } else if (dynamicSlippage < 0.02) {
								// 	ammo *= .75; // Increase buy amount if slippage is low
								// }



								// console.log('SELLING NUKE ON RAYDIUM');
								...
								// 	}
								// } else {
								// 	console.log("Error selling tokens:", response2.statusText);
								// }
								console.log('BUYING NUKE ON PUMP')
								console.log('BUYING')
								console.log('BUYING')
